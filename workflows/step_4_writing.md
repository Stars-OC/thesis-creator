...
> **写作时即嵌入 ref_id，禁止"先写完再补引用"。**
> 经历多次案例验证，写完才补引用会导致：
> - 整章重排版（破坏段落自然承接）
> - 部分引用强行塞入与段意不符的位置（被 AIGC 检测识别为模板拼接）
> - Step 7 合并后才发现引用复用，被迫整章返工
>
> 因此 Stage 2 扩写时，必须按以下章节配额边写边嵌入 `[ref_xxx]`：

| 章节类型 | 引用密度 | 推荐配额 | 引用风格 |
|----------|----------|----------|----------|
| 绪论 / 引言 | 高 | 8-12 篇 | 国内外研究现状综述、问题提出依据 |
| 关键技术 / 理论基础 | 高 | 6-10 篇 | 技术演进与原理引证 |
| 文献综述（独立章节时） | 极高 | 10-15 篇 | 主题分类综述，每个主题 2-3 篇 |
| 需求分析 / 研究设计 | 中 | 3-5 篇 | 方法/模型/调研工具的引用 |
| 系统设计 / 方法实现 | 中低 | 2-4 篇 | 模式、算法或框架的来源引用 |
| 系统实现 / 实证分析 | 低 | 1-3 篇 | 工具或库的引证、对比文献 |
| 系统测试 / 实验结果 | 极低 | 0-2 篇 | 测试方法学或评测基准引用 |
| 总结与展望 / 结语 | 0 | 0 篇 | **禁止引用**（结论必须为作者表达） |
| 摘要 / 致谢 | 0 | 0 篇 | **禁止引用** |

#### Stage 2 写作前必做（写作即引用流程）

1. **加载** `prompts/reference_citation_prompt.md`，作为 writer 系统提示一部分
2. **加载** `workspace/references/verified_references.yaml`，按本章关键词筛出候选未占用文献
3. **预分配** ref_id：写作前先选出本章拟用的 `[ref_xxx]` 列表，按段落预分配（每段最多 1 篇）
4. **边写边嵌入**：撰写正文时即在论据句尾插入 `[ref_xxx]`，不允许"先空着等会儿补"
5. **写完即更新** `workspace/cited_references.json`，标记本章已占用的 ref_id
6. **每章完成后** 校验：引用密度达标 + 无重复 ref_id + 摘要/致谢/结论无引用

> **失败案例（必须避免）**：thesis-workspace 在 2026-05-22 案例中，先写完 7 章再补 25 个 `[ref_xxx]`，
> 又被迫重新合并。后续每次写作必须严格按本节流程操作。
