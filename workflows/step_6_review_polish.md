...
> [!IMPORTANT]
> 处理 AIGC 降低请求时，必须按“处理前计划 → 场景化重写 → 结构重组 → 细节注入 → 语言去模板化 → 清单自检”的顺序执行。禁止只做同义替换，也禁止只输出改写文本而不输出自检表。
...
- 主要风险：模板词、句式均匀、条款过度对称、显性转接词、原句骨架残留
- 处理策略：场景化重写、结构重组、细节注入、语言去模板化中的具体动作
