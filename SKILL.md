...
├── .thesis-config.yaml        # ⭐ 配置文件（控制日志、导出等设置）
...
> **配置文件位置**: `thesis-workspace/.thesis-config.yaml`

**配置文件用于控制 thesis-creator 的行为，包括日志开关、导出格式等。**
...
3. **何时开启日志**：
   - 需要调试问题时 → `enabled: true`
   - 需要追踪执行过程时 → `enabled: true`
   - 正常运行时 → `enabled: false`
```
...
4. 读取 `reference/doc/` → 参考用户文献资料
5. **读取 `reference/code/` → 分析代码和技术栈** ⭐ 新增

**代码文件夹处理流程**：

```mermaid
flowchart TD
    A[扫描 reference/code/ 目录] --> B{存在文件?}
    B -->|是| C[读取所有文件]
    B -->|否| Z[跳过，输出空总结]
    C --> D[检测外部项目引用]
    D --> E{有外部引用?}
    ...
    G --> H[生成技术栈总结]
    H --> I[生成代码结构总结]
    I --> J[保存到 workspace/code_summary.md]
    J --> K[刷新上下文]
    K --> L[输出分析摘要]
```

**代码分析输出**：
- `workspace/code_summary.md` - 包含以下内容：
  - **技术栈清单**：前端/后端技术栈
  - **核心模块**：主要功能模块和职责
  - **关键接口**：API 端点和数据流
  - **代码片段**：可引用的代码示例（≤20行）
...
> - **`workspace/code_summary.md`** — 代码技术栈总结（第4、5章必须加载） ⭐ 新增
...
for 每个章节 in outline:
    0. 📖 加载必要 prompt 文件
       - 基础：aigc_reducer_prompt.md、writer_guidelines.md
       - ⭐ 第4、5章额外加载：workspace/code_summary.md（代码技术栈）
    1. 检查是否为"规定动作"章节
    2. 生成内容（同步应用 AIGC 降重策略）
       - ⭐ 第4章设计：引用 code_summary.md 的技术栈和架构信息
       - ⭐ 第5章实现：引用 code_summary.md 的代码片段（≤20行）
    3. 生成图表占位符
...
| 章节 | 可引用内容 | 引用格式 |
|------|----------|----------|
| 第4章 系统设计 | 技术栈清单、核心模块、架构图 | "本系统采用 React+Spring Boot 技术栈..." |
| 第5章 系统实现 | 代码片段（≤20行）、接口说明 | 代码块 + 设计说明 + 效果分析 |
