...
class TerminalEncodingTest(unittest.TestCase):
    ...
    def test_document_exporter_pdf_command_uses_terminal_encoding(self):
        ...
        with patch.dict(sys.modules, {"docx2pdf": None}), \
                patch("document_exporter.pdf_converter.sys.platform", "linux"), \
                patch("document_exporter.pdf_converter.subprocess.run", return_value=completed) as run:
            pdf_converter.convert_docx_to_pdf("paper.docx", "paper.pdf")
